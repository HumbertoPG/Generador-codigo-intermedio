...
def p_Factor(p):
    """Factor : INTLIT
              ...
              | '(' Expression ')' """
    if len(p) == 4: p[0] = p[2]
    elif isinstance(p[1], int): p[0] = Literal(p[1], 'INT')
    elif isinstance(p[1], float): p[0] = Literal(p[1], 'FLOAT')
    elif isinstance(p[1], str) and p.slice[1].type == 'STRING_LITERAL': p[0] = Literal(p[1], 'STRING')
    elif isinstance(p[1], str): p[0] = Variable(p[1])
    else: p[0] = p[1]
...
def p_error(p):
    ...
    else: print("Error de sintaxis: Fin de archivo inesperado")
...
class IRGenerator(Visitor):
    ...
    def cast_types(self, lhs, rhs):
        if lhs.type == rhs.type: return lhs, rhs
    ...
    def visit_block(self, node: Block):
        for decl in node.decls: decl.accept(self)
        for stmt in node.stmts: stmt.accept(self)

    def visit_declaration(self, node: Declaration):
        ty = floatType if node.type == 'float' else intType
    ...
    def visit_assignment(self, node: Assignment):
        ...
        if isinstance(val.type, ir.IntType) and isinstance(ptr.type.pointee, ir.FloatType):
            val = self.builder.sitofp(val, floatType)
        elif isinstance(val.type, ir.FloatType) and isinstance(ptr.type.pointee, ir.IntType):
    ...
    def visit_literal(self, node: Literal):
        if node.type == 'INT':
            ...
            global_fmt = ir.GlobalVariable(self.module, c_fmt.type, name=f"str_{len(self.symbol_table)}")
            global_fmt.linkage = 'internal'
            global_fmt.global_constant = True
            global_fmt.initializer = c_fmt
            fmt_ptr = self.builder.bitcast(global_fmt, voidptr_ty)
            self.stack.append(fmt_ptr)
    ...
    def visit_while(self, node: WhileNode):
        whileHead = self.current_function.append_basic_block('while-head')
        whileBody = self.current_function.append_basic_block('while-body')
        whileExit = self.current_function.append_basic_block('while-exit')
        ...
        cond = self.stack.pop()
        self.builder.cbranch(cond, whileBody, whileExit)
    ...
    def visit_if(self, node: IfNode):
        ...
        if not self.builder.block.is_terminated: self.builder.branch(ifMerge)

        self.builder.position_at_start(ifFalse)
        if node.else_body: node.else_body.accept(self)
        if not self.builder.block.is_terminated: self.builder.branch(ifMerge)

        self.builder.position_at_start(ifMerge)
